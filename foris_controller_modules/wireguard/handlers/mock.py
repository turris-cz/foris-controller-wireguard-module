...
class MockWireguardHandler(Handler, BaseMockHandler):
    ...
    def list(self):
        return [
            ...
            for k, v in MockWireguardHandler.clients.items()
        ]

    @logger_wrapper(logger)
    def set(self, id, enabled):

        if id not in MockWireguardHandler.clients:
            return False

        MockWireguardHandler.clients[id]["enabled"] = enabled
        return True

    @logger_wrapper(logger)
    def add(self, id, config):

        if id in MockWireguardHandler.clients:
            return False

        MockWireguardHandler.clients[id] = {"enabled": True, "config": config}
        return True
    ...
    def delete(self, id):
        ...
        del MockWireguardHandler.clients[id]
        return True
